...
col_patterns = ["col_name1", "col_name2"]
search_values = ["val1", "val2"]
target_schema = "public"
excluded_tables = [("public", "excluded_table_name")]
...
try:
    ...
    # Tìm tất cả cột có tên KHỚP HOÀN TOÀN với col_patterns
    col_conditions = " OR ".join(["column_name = %s" for _ in col_patterns])
    cur.execute(f"""
        SELECT table_schema, table_name, column_name
        FROM information_schema.columns
        WHERE table_schema = %s AND ({col_conditions})
        ORDER BY table_name
    """, [target_schema] + col_patterns)
    ...
    for schema, table, matched_col in candidates:
        ...
        try:
            ...
            print(f"  -> Bỏ qua bảng lỗi khi lấy header: {full_table}: {e}")
            conn.rollback()
            continue
